...
void math_track(IshieldContext context, IshieldDevice mouse,
                curve curve, point center, double t1, double t2,
                unsigned int partitioning) {
    lower_process_priority();

    IshieldMouseStroke mstroke;
    double delta = t2 - t1;
    point position = curve(t1);

    mstroke.flags = ISHIELD_MOUSE_MOVE_ABSOLUTE;

    mstroke.state = ISHIELD_MOUSE_LEFT_BUTTON_UP;
    mstroke.x = static_cast<int>((0xFFFF * center.x) / screen_width);
    mstroke.y = static_cast<int>((0xFFFF * center.y) / screen_height);
    ishield_send(context, mouse, (IshieldStroke *)&mstroke, 1);
    ...
    for (unsigned int i = 0, j = 0; i <= partitioning + 2; ++i, ++j) {
        if (j % 250 == 0) {
            ...
            busy_wait(25 * milliseconds);
            mstroke.state = ISHIELD_MOUSE_LEFT_BUTTON_DOWN;
            ishield_send(context, mouse, (IshieldStroke *)&mstroke, 1);
            mstroke.state = 0;
...
int main() {
    ...
    ishield_set_filter(context, ishield_is_keyboard,
                            ISHIELD_FILTER_KEY_DOWN | ISHIELD_FILTER_KEY_UP);
    ishield_set_filter(context, ishield_is_mouse,
                            ISHIELD_FILTER_MOUSE_MOVE);
    ...
    while (ishield_receive(context, device = ishield_wait(context), &stroke, 1) > 0) {
        if (ishield_is_mouse(device)) {
            if (!mouse) {
                mouse = device;
                cout << "Impersonating mouse " << device - ISHIELD_MOUSE(0) << ".\n\n";
